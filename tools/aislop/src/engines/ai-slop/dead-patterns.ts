...
const detectTodoStubs = (content: string, relativePath: string): Diagnostic[] => {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (TODO_PATTERN.test(trimmed)) {
			// An item that references a tracking issue or AGENT marker is documented debt, not an unfinished stub.
			...
			diagnostics.push(
				slop(
					...
					"ai-slop/todo-stub",
					"info",
					"Unresolved TODO/stub comment without agent tracking",
					"Resolve the TODO or track it with '// AGENT:STUB [scope]: reason' or '// AGENT:MOCK [scope]: reason'",
					false,
				),
			);
		}
...
const UNMARKED_MOCK_PATTERN =
	/^\s*(?:export\s+)?(?:const|let|var)\s+((?:mock|dummy|fake|sample|stub|demo|placeholder)[A-Za-z0-9_]*|[A-Za-z0-9_]*(?:Mock|Dummy|Fake|Sample|Stub|Demo|Placeholder)|test(?:Data|Users?|Items?|List|Payload|Tasks?))\b\s*[:=]/;

const detectUnmarkedMocks = (content: string, relativePath: string): Diagnostic[] => {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (UNMARKED_MOCK_PATTERN.test(line)) {
			...
			if (!line.includes("AGENT:MOCK") && !prevLine.includes("AGENT:MOCK")) {
				diagnostics.push(
					slop(
						relativePath,
						i + 1,
						...
						"info",
						"Unmarked mock data variable in production source code",
						"Tag with '// AGENT:MOCK [scope]: reason -> real_service' or connect real service",
						false,
					),
				);
			}
